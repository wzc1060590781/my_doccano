...
export default {
  ...
  created() {
    HTTP.get('statistics').then((response) => {
      this.labelData = this.makeData(response.data.label, '标签状态');
      this.userData = this.makeData(response.data.user, '用户状态');
